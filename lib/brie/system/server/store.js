...
Store.prototype.createBlobFromStream = function(stream, callback) {
	var proc = this.callGit(['hash-object', '-w', '--stdin'])
	var id = '';
...
if (module.parent === null) {
	...
	if (args[0] == 'create-blob') {
		...
		store.createBlobFromStream(process.stdin, function(id, err) {
			if (err) {
		...
		console.log('Actions: create-blob, read-blob');
		process.exit(1);
	}
}
