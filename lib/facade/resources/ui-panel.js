...
function Panel( $selection, options ) {
	...
	function page( diff ) {
		...
		var ctop = Math.max( Math.min( c.top - ( p.height * diff ), 0 ), -( c.height - p.height ) );
		// Adjust handle position 
		var htop = Math.max( Math.min( h.top + ( h.height * diff ), t.height - h.height ), 0 );
