	...
	'can': {
		'initialize': function( options ) {
			...
			this.$.click( function() {
				...
				that._options.overlays.item( that._overlay ).$.parent()
					.css( { 'top': offset.top + that.$.outerHeight(), 'left': offset.left } )
					.show();
			} );
			this.list.$.click( function() {
				that._options.overlays.item( that._overlay ).$.parent().hide();
			} );
		}
	}
} );
